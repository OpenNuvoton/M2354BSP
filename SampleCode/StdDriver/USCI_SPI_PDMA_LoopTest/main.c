...
void SYS_Init(void)
{
    ...
    SYS->GPD_MFPL &= ~(SYS_GPD_MFPL_PD0MFP_Msk | SYS_GPD_MFPL_PD1MFP_Msk | SYS_GPD_MFPL_PD2MFP_Msk | SYS_GPD_MFPL_PD4MFP_Msk);
    SYS->GPD_MFPL |= (SYS_GPD_MFPL_PD0MFP_USCI0_CLK | SYS_GPD_MFPL_PD1MFP_USCI0_DAT0 | SYS_GPD_MFPL_PD2MFP_USCI0_DAT1 | SYS_GPD_MFPL_PD4MFP_USCI0_CTL0);
...
void UsciSpiLoopTest_WithPDMA(void)
{
    ...
    {
        s_au16MasterToSlaveTestPattern[u32DataCount] = (uint16_t)(0x5500 | (u32DataCount + 1));
        s_au16SlaveToMasterTestPattern[u32DataCount] = (uint16_t)(0xAA00 | (u32DataCount + 1));
    }
        ...
        while(1)
        {
            ...
            if(u32RegValue & PDMA_INTSTS_TDIF_Msk)
            {
                        ...
                        ((1 << USPI_MASTER_TX_DMA_CH) | (1 << USPI_MASTER_RX_DMA_CH) | (1 << USPI_SLAVE_TX_DMA_CH) | (1 << USPI_SLAVE_RX_DMA_CH)))
                {
                    ...
                    for(u32DataCount = 0; u32DataCount < TEST_COUNT; u32DataCount++)
                    {
                        if(s_au16MasterToSlaveTestPattern[u32DataCount] != s_au16SlaveRxBuffer[u32DataCount])
                        {
                            i32Err = 1;
                            break;
                        }
                        if(s_au16SlaveToMasterTestPattern[u32DataCount] != s_au16MasterRxBuffer[u32DataCount])
                        {
                            i32Err = 1;
                            break;
                        }
                    ...
                    for(u32DataCount = 0; u32DataCount < TEST_COUNT; u32DataCount++)
                    {
                        s_au16MasterToSlaveTestPattern[u32DataCount]++;
                        s_au16SlaveToMasterTestPattern[u32DataCount]++;
                    }
                    /* Re-trigger */
