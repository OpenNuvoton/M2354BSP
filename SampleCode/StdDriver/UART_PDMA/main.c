...
/* Global variables                                                                                        */
/*---------------------------------------------------------------------------------------------------------*/
static int32_t g_i32UartTestLength = 64;
...
void PDMA_UART_TxTest(void)
{
    ...
    PDMA_SetTransferCnt(PDMA0, UART_TX_DMA_CH, PDMA_WIDTH_8, (uint32_t)g_i32UartTestLength);
...
void PDMA_UART_RxTest(void)
{
    /* UART Rx PDMA channel configuration */
    /* Set transfer width (8 bits) and transfer count */
    PDMA_SetTransferCnt(PDMA0, UART_RX_DMA_CH, PDMA_WIDTH_8, (uint32_t)g_i32UartTestLength);
...
void PDMA0_IRQHandler(void)
{
    ...
    else if(u32Status & PDMA_INTSTS_TDIF_Msk) /* Transfer Done */
    {
        ...
        if(PDMA_GET_TD_STS(PDMA0) & (1 << UART_TX_DMA_CH))
        {
            ...
            UART_PDMA_DISABLE(UART1, UART_INTEN_TXPDMAEN_Msk);
        }
...
void PDMA_UART(int32_t i32option)
{
    ...
    BuildSrcPattern((uint32_t)g_au8SrcArray, (uint32_t)g_i32UartTestLength);
    ClearBuf((uint32_t)g_au8DestArray, (uint32_t)g_i32UartTestLength, 0xFF);
    ...
    /* Enable UART Tx and Rx PDMA function */
    if(g_u32TwoChannelPdmaTest == 1)
        UART_PDMA_ENABLE(UART1, UART_INTEN_TXPDMAEN_Msk);
    else
        UART_PDMA_DISABLE(UART1, UART_INTEN_TXPDMAEN_Msk);
