...
static volatile uint8_t g_au8RxData[4];
static volatile uint16_t g_u16RecvAddr;
static volatile uint8_t g_u8DataLenS;
static volatile uint8_t g_u8SlvPWRDNWK = 0, g_u8SlvI2CWK = 0;
static volatile uint32_t g_u32WKfromAddr;

static enum UI2C_SLAVE_EVENT s_Event;
    ...
    else if((u32Status & UI2C_PROTSTS_ACKIF_Msk) == UI2C_PROTSTS_ACKIF_Msk)
    {
        ...
        else if(s_Event == SLAVE_GET_DATA)
        {
            ...
            if(g_u8DataLenS == 2)
            {
                ...
                slave_buff_addr = (uint32_t)(g_au8RxData[0] << 8) + g_au8RxData[1];
            }
            if(g_u8DataLenS == 3)
            {
