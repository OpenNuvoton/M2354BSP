...
static char  gZ[580];                         /* temporary buffer used to keep output secret Z        */

static char  g_dSrc[580];                      /* one's own private key                        */
static char  g_QxSrc[580];                     /* the party's public key 1                     */
static char  g_QySrc[580];                     /* the party's public key 2                     */
static char  g_ZSrc[580];                     /* the party's public key 2                     */
...
int32_t main(void)
{
    ...
    for(p = 0; p < (int32_t)(sizeof(gPattern) / sizeof(ECDH_VECTOR_T)); p++)
    {
        printf("Run ECC CDH pattern %s => ", gPattern[p].vecotr_name);

        memcpy(g_dSrc,  gPattern[p].d,  sizeof(g_dSrc));
        memcpy(g_QxSrc, gPattern[p].Qx,  sizeof(g_QxSrc));
        memcpy(g_QySrc, gPattern[p].Qy,  sizeof(g_QySrc));

        if(ECC_GenerateSecretZ(CRPT, gPattern[p].curve, (char *)g_dSrc, (char *)g_QxSrc, (char *)g_QySrc, gZ) < 0)
        {
            printf("ECC CDH secret Z generation failed!!\n");
            goto lexit;
        }

        memcpy(g_ZSrc, gPattern[p].Z,  sizeof(g_ZSrc));

        /* truncate leading zeros                */
        for(pz = &gZ[0]; *pz == '0'; pz++);

        /* truncate leading zeros  */
        for(pZ = (char *)&g_ZSrc; *pZ == '0'; pZ++);
