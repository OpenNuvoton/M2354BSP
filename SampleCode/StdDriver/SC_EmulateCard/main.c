...
static volatile SC_INFO_T g_sSCInfo;
static volatile uint8_t g_au8SCTxBuf[300] = {0};
static volatile uint8_t g_au8SCRxBuf[300] = {0};
static volatile uint8_t g_au8CardATRBuf[48] = {0};


#if (SEL_PROTOCOL_T == 0UL)
static const uint8_t g_au8CardATR_T0[] =
{
...
static const uint8_t g_au8CardATR_T1[] =
{
    0x3B,
    0xF7,
    0x11,   /* No PPS exchange */
...
/* Fi */
static const CLOCK_RATE_CONVERSION g_ClockRateConversion[] =
{
    { 372,  4000000     },
    { 372,  5000000     },
    { 558,  6000000     },
...
/* Di */
static const BIT_RATE_ADJUSTMENT g_BitRateAdjustment[] =
{
    { 0,    0   },
    { 1,    1   },
    { 2,    1   },
...
static void Init_CardInfo_And_CardPin(SC_INFO_T *sc_info, SC_T *sc, uint8_t u8Protocol)
{
    uint32_t u32SCRxBuf, u32SCTxBuf;
    /* Card info. */
    {
        sc_info->sc         = sc;
    ...
    u32SCRxBuf = (uint32_t)&g_au8SCRxBuf;
    u32SCTxBuf = (uint32_t)&g_au8SCTxBuf;
    /* Clear Rx/Tx buffer */
    memset((void *)u32SCRxBuf, 0x0, sizeof(g_au8SCRxBuf));
    memset((void *)u32SCTxBuf, 0x0, sizeof(g_au8SCRxBuf));
}


#if defined(SC_CLK_MODE_AUTO)
static int32_t Card_AutoFreqCheck(void)
{
...
static void Card_SetFreqAfterPPSExchange(uint32_t u32Fi, uint32_t u32Di)
{
    (void)u32Di;
...
static int32_t Card_ResponseATR(SC_INFO_T *sc_info)
{
    uint8_t i;
    uint8_t u32Fi, u32Di, u32Len;
    uint8_t u8PCK = 0;
    SC_T *sc;
    uint32_t u32CardATRBuf, u32SCRxBuf;
    ...
    if(sc_info->u8Protocol == 0)
    {
        ...
        u32CardATRBuf = (uint32_t)&g_au8CardATRBuf[0];
        memcpy((void *)u32CardATRBuf, g_au8CardATR_T0, u32Len);
    }
    else
    {
    ...
    if(sc_info->u8Protocol == 1)
    {
        ...
        u32CardATRBuf = (uint32_t)&g_au8CardATRBuf[0];
        memcpy((void *)u32CardATRBuf, g_au8CardATR_T1, u32Len);
    }
    else
    {
        printf("\nERROR. Unknow ptotocol.\n");
    ...
    u32SCRxBuf = (uint32_t)g_au8SCRxBuf[0];

    sc_info->OP_State = SC_OP_WRITE_ATR;
    sc_info->pu8TxBuf = (uint8_t *)u32CardATRBuf;
    sc_info->u32TxLen = u32Len;
    sc_info->u32TxPos = 0;
    sc_info->pu8RxBuf = (uint8_t *)u32SCRxBuf;
    sc_info->u32RxLen = 0;
    sc_info->u32RxPos = 0;
    ...
    if(sc_info->OP_State == SC_OP_PPS_EXCHANGE)
    {
        ...
        sc_info->pu8TxBuf = (uint8_t *)u32SCRxBuf;
...
static int32_t Check_IsValidT0Cmd(SC_T *sc)
{
    ...
    uint8_t u32INS, u32P1, u32P2; // remove u32CLA and u32P3
...
static void ProcessIRQ_TxRx(SC_T *sc)
{
    uint32_t i;
    uint32_t u32Tmp;
    char c;
...
static const uint8_t g_u8RespMF[] =
{
...
static const uint8_t g_u8RespDF[] =
{
    0xDF, 0x0A, 0x11, 0x22, 0x33,
    0x44, 0x55, 0x66, 0x77, 0x88,
};

static const uint8_t g_u8RespEF[] =
{
...
static int32_t Process_T0(SC_INFO_T *sc_info)
{
    SC_T *sc;
    uint8_t i;
    uint8_t u32SW1, u32SW2, u32RespLen;
    //uint8_t u8CLA, u8INS, u8P1, u8P2, u8P3, u8D[8];
    uint8_t u8INS, u8P1, u8P2, u8P3, u8D[8]; // remove u8CLA
    uint32_t u32SCTxBuf, u32RespMF, u32RespDF, u32RespEF;
    ...
    do
    {
        u32SCTxBuf = (uint32_t)&g_au8SCTxBuf[0];
...
static volatile uint32_t g_u32FileID = 0;
static volatile uint32_t g_u32MaxFileSize = 0;
static volatile uint8_t g_au8FileBuf[256];
static int32_t Process_T1(SC_INFO_T *sc_info)
{
    SC_T *sc;
    uint32_t u32RxLen = 0, u32TxLen = 0;
    //uint8_t u8NAD, u8PCB, u8LEN, u8CLA, u8INS, u8EDC = 0;
    uint8_t u8PCB, u8INS, u8EDC = 0;
    uint8_t u8Data, u8RespIFLen = 0, u8InDataLen;
    uint8_t *pu8RxBuf, *pu8TxBuf;
    uint8_t i;

    sc = sc_info->sc;
    u32RxLen = sc_info->u32RxLen;
    pu8RxBuf = (uint8_t *)((uint32_t)&g_au8SCRxBuf[0]);
    pu8TxBuf = (uint8_t *)((uint32_t)&g_au8SCTxBuf[0]);
    ...
    do
    {
        ...
        if(u8PCB == SC_T1_BLOCK_S_IFS_REQ)
        {
            ...
            memcpy((void *)((uint32_t)g_au8SCTxBuf), (void *)((uint32_t)g_au8SCRxBuf), u32TxLen);
            /* u8PCB */
        ...
        if(g_sSCInfo.i32CmdStatus < 0)
        {
            ...
            /* NAD, PCB */
            memcpy((void *)((uint32_t)g_au8SCTxBuf), (void *)((uint32_t)g_au8SCRxBuf), 2);
            /* LEN */
            g_au8SCTxBuf[2] = u8RespIFLen;
            /* INFO FIELD */
            memcpy((void *)((uint32_t)&g_au8SCTxBuf[3]), (void *)((uint32_t)g_au8FileBuf), u8RespIFLen - 2);
            /* Calculate EDC */
            g_au8SCTxBuf[u32TxLen - 1] = 0x0;
            for(i = 0; i < (u32TxLen - 1); i++)
                g_au8SCTxBuf[u32TxLen - 1] ^= pu8TxBuf[i];
    ...
    sc_info->OP_State = SC_OP_WRITE;
    sc_info->pu8TxBuf = (uint8_t *)((uint32_t)&g_au8SCTxBuf[0]);
...
static void Card_SetFreqForATR(void)
{
#if defined(SC_CLK_MODE_AUTO)
...
int main(void)
{
    ...
    u32SCInfo = (uint32_t)&g_sSCInfo;
    /* Init card info and I/O pins */
    Init_CardInfo_And_CardPin((SC_INFO_T *)u32SCInfo, SC0, SEL_PROTOCOL_T);
    ...
    while(1)
    {
        if(g_sSCInfo.u8RestToATR == 1)
        {
            ...
            if(Card_ResponseATR((SC_INFO_T *)u32SCInfo) < 0)
            {
                g_sSCInfo.u8RestToATR = 0;
        ...
        if(g_sSCInfo.u8RestToATR == 0x5A)
        {
            ...
            {
                if(Process_T0((SC_INFO_T *)u32SCInfo) < 0)
                {
                    sc->CTL |= (SC_CTL_RXOFF_Msk | SC_CTL_TXOFF_Msk);
                    g_sSCInfo.OP_State = SC_OP_IDLE;
                    g_sSCInfo.u8RestToATR = 0;
            ...
            {
                if(Process_T1((SC_INFO_T *)((uint32_t)&g_sSCInfo)) < 0)
                {
                    sc->CTL |= (SC_CTL_RXOFF_Msk | SC_CTL_TXOFF_Msk);
                    g_sSCInfo.OP_State = SC_OP_IDLE;
                    g_sSCInfo.u8RestToATR = 0;
            ...
            g_sSCInfo.pu8TxBuf = (uint8_t *)u32SCRxBuf;
            g_sSCInfo.u32TxLen = sizeof(g_au8SCRxBuf);
            g_sSCInfo.u32TxPos = 0;
            g_sSCInfo.pu8RxBuf = (uint8_t *)u32SCRxBuf;
