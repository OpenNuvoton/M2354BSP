 ...
 * $Revision: 1 $
 * $Date: 20/07/07
 * @brief    Bank Swap sample code(Bank0 Loader).
 *
 * @note
...
/* Background program length  */
#define DB_PROG_LEN        (4 * FMC_FLASH_PAGE_SIZE)
/* Loop count */
#define CRC32_LOOP_CNT     50
#define TXBUFSIZE          0x16f4
...
void UART_Init(UART_T* uart, uint32_t u32ClkSrc, uint32_t u32ClkDiv, uint32_t u32BaudRate)
{
    ...
    CLK->CLKSEL2 = (CLK->CLKSEL2 & (~CLK_CLKSEL2_UART1SEL_Msk)) | (u32ClkSrc << CLK_CLKSEL2_UART1SEL_Pos);
