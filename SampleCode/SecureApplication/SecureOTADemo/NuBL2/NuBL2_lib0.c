...
int32_t NuBL2_ExecuteVerifyNuBL3x(uint32_t *buf, int32_t mode)
{
    ...
    uint32_t            *keybuf, *infobuf, base, len, start, end, u32FwInfoAddr, u32PubKeyAddr;
    ECC_PUBKEY_T        PubKey;
    FW_INFO_T           FwInfo;
    ...
    if(buf == NULL)
    {
        u32FwInfoAddr = (uint32_t)&FwInfo;
        infobuf = (uint32_t *)u32FwInfoAddr;
    ...
    u32PubKeyAddr = (uint32_t)&PubKey;
    keybuf = (uint32_t *)u32PubKeyAddr;
    for(i = 0; i < (len / 4); i++)
...
{
    volatile int32_t    ret = -1000;
    uint32_t            *keybuf, base, len, u32PubKeyAddr;
    ECC_PUBKEY_T        PubKey;
    uint32_t            AESkey[8];
    volatile uint32_t    i;
    ...
    SetECCRegisters(0x00020000, NULL, keybuf);
    ...
    /* Get ECDH shared key */
    u32EccX1Addr = (uint32_t)&CRPT->ECC_X1[0];
    pu32EccX1Addr = (uint32_t *)u32EccX1Addr;
    memcpy(Key32, pu32EccX1Addr, sizeof(AESkey));
    ...
    /* Get ECDH shared key */
    memcpy(Key33, pu32EccX1Addr, sizeof(AESkey));
...
{
    volatile int32_t    ret = -1000;
    uint32_t            *keybuf, base, len, start, end, u32PubKeyAddr, u32EccX1Addr, *pu32EccX1Addr;
    ECC_PUBKEY_T        PubKey;
    uint32_t            AESkey[8], Hash[8], HashValue[8];
    volatile uint32_t    i;
...
int32_t IdentifyPublicKey(uint32_t *p32Buf, int32_t i32Mode)
{
    ...
    uint32_t            *keybuf, base, len, start, end, u32PubKeyAddr;
