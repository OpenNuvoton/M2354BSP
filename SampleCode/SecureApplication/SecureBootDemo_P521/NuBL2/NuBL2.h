...
typedef struct
{
    uint32_t        au32Key0[17];    /* up to 544-bits */
    uint32_t        au32Key1[17];    /* up to 544-bits */
...
typedef struct
{
    uint32_t        au32R[17];   /* up to 544-bits */
    uint32_t        au32S[17];   /* up to 544-bits */
...
typedef struct
{
    ...
    uint32_t        au32FwHash[16];     /* 64-bytes (512-bits) */

    ECDSA_SIGN_T    sign;               /* 68-bytes R + 68-bytes S */
